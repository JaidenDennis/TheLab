...
- **Task 4.** The journal's encoder had four defects, all from the task's own code: payload kwargs could silently overwrite the clock-sourced `ts`; naive datetimes were accepted; nested pydantic models rendered datetimes as RFC3339 `Z`, which `datetime.fromisoformat` cannot parse before 3.11 while bare datetimes rendered `+00:00`; and `NaN`/`Infinity` produced invalid JSON. Fixed with a reserved-key guard, a shared public `models.require_utc`, `model_dump(mode="python")`, and `allow_nan=False`.
- **Task 5.** `BarAggregator` accepted duplicate timeframes and silently doubled that timeframe's volume; the constructor now rejects them. Separately, the close-time/duration ordering tie-break was unfalsifiable through the public API — mutating the sort key left every test green, because `_ordered`'s input is already ordered by construction. Added a white-box test passing shuffled input, verified by mutation.
